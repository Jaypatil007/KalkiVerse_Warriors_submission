...
# Import all your Firestore functions and the initializer
...
# Adjust path assuming .env is in trade_orchestrator/
orchestrator_root = Path(__file__).parent.parent.resolve()
...
logging.basicConfig(
    level=os.environ.get("LOG_LEVEL", "INFO").upper(), # Use env var for overall level
...
# Initialize Firestore client once when the server starts
try:
    _initialize_firestore_client()
    logging.info("Firestore client verified for use by MCP server.")
except Exception as e:
    logging.critical(f"Failed to initialize Firestore client on startup: {e}. Exiting server.", exc_info=True)
    # The server process will exit if _initialize_firestore_client raises an error


# --- Wrap Firestore functions as ADK FunctionTools ---
# Each function will be exposed as a distinct tool.
ADK_FIRESTORE_TOOLS = {
...
logging.info(f"Defined {len(ADK_FIRESTORE_TOOLS)} Firestore tools.")
...
async def list_mcp_tools() -> list[mcp_types.Tool]:
    ...
    for tool_name, adk_tool_instance in ADK_FIRESTORE_TOOLS.items():
        # Ensure the tool name is set, sometimes FunctionTool doesn't infer it automatically
